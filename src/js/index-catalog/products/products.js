function goodsOut(data) {
  ...
  for (var key in data) {
    ...
    // out+='<button class="products__button">Быстрый просмотр</button>';
    // out+='</div>';
    // out+='</div>';
    ...
    out += `<p class="products__small-text">${data[key].description}</p>`;
    out += '<div class="products__bottom-content">';
    out += `<button class="products__button" data-id="${key}" data-cost="${data[key].cost}">Быстрый просмотр</button>`;
    out += '</div>';
    out += '</div>';
  }
  $('.products').html(out);
